...
const SignIn = () => {
  ...
  // **Change 1**: Refactored Redirect logic
  if (isLogged) return <Redirect href="/" />;
  if (loading) return null; // Ensures no unnecessary rendering during loading

  const handleLogin = async () => {
    try {
      // **Change 2**: Improved error handling with try-catch
      const result = await login();
